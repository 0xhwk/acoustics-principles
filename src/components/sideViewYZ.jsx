...
export const SideViewYZ = ({
  ...
  const handleMouseMove = (event) => {
    ...
    if (draggingIndex == "source") {
      const oldX = sourcePoint[0];
      setSourcePoint([oldX, coordinates.x, coordinates.y]);
    }

    if (draggingIndex == "listener") {
      const oldX = listenerPoint[0];
      setListenerPoint([oldX, coordinates.x, coordinates.y]);
    }
    ...
    if (
      ...
      const oldX = topPointMatrix[draggingIndex][0];
      newTopPointMatrix[draggingIndex] = [oldX, coordinates.x, coordinates.y];
  ...
  const createPoint = () => {
    ...
    const newPoint = [0, mousePosition.x, mousePosition.y];
  ...
  const renderSourcePoint = () => {
    ...
    const left = `${boundaryCoordX + y * scale}px`;
  ...
  const renderPolygon = () => {
    const pointGroups = generateRectangularPointGroups(topPointMatrix, "YZ");
    console.log("toppointmatrix", topPointMatrix);
    console.log("GROUPS", pointGroups);

    const polygonGroup = pointGroups.map((pointGroup) => {
      const points = pointGroup
        .map(
          ([x, y, z]) =>
            `${boundaryCoordX + y * scale},${boundaryCoordY - z * scale}`
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 m-2 text-white">
        Mouse Position: {`Y: ${mousePosition.x}, Z: ${mousePosition.y}`}
      </div>
    </div>
  );
};
